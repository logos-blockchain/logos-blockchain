...
fn transform_op(op_ref: &OpRef<'_>, event: Option<TxEventPayload>) -> Option<WalletOp> {
    match *op_ref {
        OpRef::Transfer(transfer) => Some(WalletOp::Transfer(transfer.clone())),
        OpRef::ChannelDeposit(deposit) => Some(WalletOp::ChannelDeposit(deposit.clone())),
        ...
        OpRef::SDPDeclare(declaration) => Some(WalletOp::Lock(declaration.locked_note_id)),
