...
pub enum Event {
    ...
    /// has been verified.
    Message(
        Box<SessionBoundEncapsulatedMessageWithVerifiedSignature>,
        PeerId,
    ),
...
impl<ObservationWindowClockProvider> Behaviour<ObservationWindowClockProvider> {
    ...
    pub fn publish_message_with_validated_header(
        ...
        if message.session() != self.current_session_info.1 {
            let Some(old_session) = &mut self.old_session else {
                return Err(SendError::InvalidSession);
            };
            return old_session.publish_message_with_validated_header(message);
        }
        let (inner_message, _) = message.into_components();
        self.forward_maybe_excluding(&inner_message.into(), None)
    }

    pub fn publish_message_with_validated_signature(
        &mut self,
        ...
        self.forward_maybe_excluding(message.as_ref(), None)
    }
    ...
    pub fn forward_message_with_validated_signature(
        ...
        self.forward_maybe_excluding(message.as_ref(), Some(except))
    }
