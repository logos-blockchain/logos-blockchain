...
#[test]
fn verify_header_signature_succeeds() {
    const PAYLOAD_BODY: &[u8] = b"hello";
    ...
    drop(msg.verify_header_signature().unwrap());
    // The message identity must be preserved.
    let (inputs2, _) = generate_inputs(2);
    let msg2 = EncapsulatedMessage::from(EncapsulatedMessageWithVerifiedPublicHeader::new(
        &inputs2,
        PayloadType::Data,
        PAYLOAD_BODY.try_into().unwrap(),
    ));
    // Just verify we can call this and get back a valid result.
    drop(msg2.verify_header_signature().unwrap());

    // The id of the signature-verified message should match the original.
    let (inputs3, _) = generate_inputs(2);
    let verified = EncapsulatedMessageWithVerifiedPublicHeader::new(
        &inputs3,
        PayloadType::Data,
        PAYLOAD_BODY.try_into().unwrap(),
    );
    let msg3 = EncapsulatedMessage::from(verified.clone());
    let sig_verified3 = msg3.verify_header_signature().unwrap();
    assert_eq!(sig_verified3.id(), verified.id());
}

#[test]
fn verify_header_signature_fails_with_invalid_signature() {
    const PAYLOAD_BODY: &[u8] = b"hello";
    ...
    let result = msg_with_invalid_signature.verify_header_signature();
    assert!(matches!(result, Err(Error::SignatureVerificationFailed)));
}

/// Tests two-step verification: signature verification succeeds, then PoQ
/// verification succeeds. This is the intended "forward then verify" path.
#[test]
fn two_step_verify_signature_then_poq_succeeds() {
    const PAYLOAD_BODY: &[u8] = b"hello";
    let verifier = NeverFailingProofsVerifier;
    ...
    let sig_verified_id = sig_verified.id();

    // Step 2: verify PoQ
    let fully_verified = sig_verified.verify_proof_of_quota(&verifier).unwrap();

    // The message identity should be preserved across both verification steps.
    assert_eq!(fully_verified.id(), sig_verified_id);
}

#[test]
fn two_step_verify_signature_succeeds_but_poq_fails() {
    use lb_blend_proofs::quota::Error as PoQError;
    ...
    // Step 1: signature verification passes
    let sig_verified = msg.verify_header_signature().unwrap();

    // Step 2: PoQ verification fails
    let result = sig_verified.verify_proof_of_quota(&verifier);
    assert!(matches!(
        result,
        Err(Error::ProofOfQuotaVerificationFailed(
            PoQError::InvalidProof
...
/// Tests that the two-step verification path produces the same
/// `EncapsulatedMessageWithVerifiedPublicHeader` as the combined
/// `verify_public_header` path, ensuring consistency.
#[test]
fn two_step_and_combined_produce_equivalent_results() {
    const PAYLOAD_BODY: &[u8] = b"hello";
    let verifier = NeverFailingProofsVerifier;
    ...
    let unverified = EncapsulatedMessage::from(msg.clone());

    // Combined path
    let combined_result = unverified.clone().verify_public_header(&verifier).unwrap();

    // Two-step path
    let two_step_result = unverified
        .verify_header_signature()
        .unwrap()
        .verify_proof_of_quota(&verifier)
        .unwrap();

    assert_eq!(combined_result, two_step_result);
}
