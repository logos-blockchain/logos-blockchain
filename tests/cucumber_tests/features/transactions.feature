Feature: Transactions
  ...
  Scenario: Funding wallets funds are managed
    Given the genesis block has the following wallet resources:
      ...
      | 1             | 0           | 0            |
      | 2             | 0           | 0            |
    And we have a sponsored genesis fee account with 20 tokens of 500 value each
    ...
    When wallet "NODE_1_WALLET" has 100 or more LGO in 10 seconds
    When wallet "NODE_2_WALLET" has 100 or more LGO in 10 seconds
    And I send 1 transactions of 100 LGO each from wallet "NODE_1_WALLET" to wallet "WALLET_1A"
    And I send 1 transactions of 100 LGO each from wallet "NODE_2_WALLET" to wallet "WALLET_2A"
    # Now we can split the funds in the user wallets
    When wallet "WALLET_1A" has 100 or more LGO in 120 seconds
    And wallet "WALLET_2A" has 100 or more LGO in 120 seconds
    And I perform 1 coin split transactions for each user wallet with 7 outputs of 10 LGO each
    ...
    When I send 5 transactions of 10 LGO each from wallet "WALLET_1A" to wallet "NODE_1_WALLET"
    And I send 5 transactions of 10 LGO each from wallet "WALLET_2A" to wallet "NODE_2_WALLET"
    Then wallet "NODE_1_WALLET" has 5 or more outputs in 120 seconds
    And wallet "NODE_2_WALLET" has 5 or more outputs in 120 seconds
