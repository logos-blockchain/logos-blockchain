...
async fn sdp_blend_activity() {
    ...
    // value, proving activity messages were submitted and accepted. We don't
    // assert on the declaration set itself: inactive declarations now persist
    // indefinitely, so mere presence in the set would not prove activity.
