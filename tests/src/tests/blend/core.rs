...
async fn blend_core_test(topology: &Topology) {
...
/// Tests Blend in Core mode: all 4 nodes are declared as SDP blend providers
/// and therefore operate in Core mode (`membership.size() >=
/// minimum_network_size` and `membership.contains_local()` is true for each
/// node).
///
/// Verifies that the network reaches consensus and makes forward progress
/// when the full blend core is active.
#[tokio::test]
#[serial]
async fn blend_core() {
    // All 4 nodes are registered as SDP blend providers, so all run in Core mode.
    let topology = Topology::spawn(
        TopologyConfig::n_validators_with_m_blend_node(4, 4),
        Some("blend_core_e2e"),
    )
    .await;
    blend_core_test(&topology).await;
}
