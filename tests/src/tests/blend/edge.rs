...
async fn blend_edge_test(topology: &Topology) {
...
/// Tests Blend in Edge mode: nodes 0–1 are declared as SDP blend providers
/// (Core mode), while nodes 2–3 have no SDP declaration and therefore operate
/// in Edge mode (`membership.size() >= minimum_network_size` but
/// `membership.contains_local()` is false).
///
/// Verifies that consensus progress is made when core and edge nodes
/// collaborate.
#[tokio::test]
#[serial]
async fn blend_edge() {
    // 4 total nodes, 2 declared as SDP blend providers (Core), 2 without
    // declaration (Edge).
    let topology = Topology::spawn(
        TopologyConfig::n_validators_with_m_blend_node(4, 2),
        Some("blend_edge_e2e"),
    )
    .await;
    blend_edge_test(&topology).await;
}
