...
/// Spins up a local test network for manual TUI testing.
///
/// Run with:
///   cargo test -p logos-blockchain-tests --test test_zone_sdk_e2e local_devnet
/// -- --ignored --nocapture
///
/// Then in another terminal:
///   cargo run -p logos-blockchain-tui-zone -- --node-url http://127.0.0.1:<port>
#[tokio::test]
#[serial]
#[ignore]
async fn local_devnet() {
    init_tracing();
    let (configs, genesis_tx) = create_general_configs(2, Some("local_devnet"));
    let deployment_settings = e2e_deployment_settings_with_genesis_tx(genesis_tx);
    let configs: Vec<_> = configs
        .into_iter()
        .map(|c| {
    ...
    println!();
    println!("Local devnet running:");
    println!("  Node URL: {}", validators[0].url());
    println!();
    println!(
        "  cargo run -p logos-blockchain-tui-zone -- --node-url {}",
        validators[0].url()
    );
    println!();
