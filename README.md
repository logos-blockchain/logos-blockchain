...
```bash
CONSENSUS_SLOT_TIME=1 logos-blockchain-node nodes/node/config-one-node.yaml --dev-mode-reset-chain-clock
```
...
CONSENSUS_SLOT_TIME=1 target/debug/logos-blockchain-node nodes/node/config-one-node.yaml
```
...
CONSENSUS_SLOT_TIME=1 cargo test --all-features local_testnet_one_node -- --ignored --nocapture
```
