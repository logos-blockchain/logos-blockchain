...
impl LogosBlockchainNode {
    ...
    pub(crate) fn stop(self) -> OperationStatus {
        ...
        if let Err(e) = runtime_handle.block_on(overwatch_handle.stop_all_services()) {
            tracing::error!("Could not stop services: {e}");
...
impl Drop for LogosBlockchainNode {
    fn drop(&mut self) {
        if self.overwatch.is_null() {
            tracing::error!("Attempted to drop a null overwatch pointer. This is a bug");
        }
        if self.runtime.is_null() {
            tracing::error!("Attempted to drop a null tokio runtime pointer. This is a bug");
        }
