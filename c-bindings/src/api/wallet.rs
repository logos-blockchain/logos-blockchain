...
pub unsafe extern "C" fn get_known_addresses(
    ...
    if node.is_null() {
        tracing::error!("[get_known_addresses] Received a null `node` pointer. Exiting.");
...
pub(crate) fn get_balance_sync(
    ...
    let Ok(runtime) = tokio::runtime::Runtime::new() else {
        tracing::error!("[Failed]to create tokio runtime. Aborting.");
...
pub unsafe extern "C" fn get_balance(
    ...
    if node.is_null() {
        tracing::error!("[get_balance] Received a null `node` pointer. Exiting.");
        return BalanceResult::from_error(OperationStatus::NullPointer);
    }
    if wallet_address.is_null() {
        tracing::error!("[get_balance] Received a null `wallet_address` pointer. Exiting.");
        return BalanceResult::from_error(OperationStatus::NullPointer);
    }
...
pub(crate) fn transfer_funds_sync(
    ...
    let Ok(runtime) = tokio::runtime::Runtime::new() else {
        tracing::error!("[transfer_funds_sync] Failed to create tokio runtime. Aborting.");
        return Err(OperationStatus::RuntimeError);
    };
...
pub unsafe extern "C" fn transfer_funds(
    ...
    if node.is_null() {
        tracing::error!("[transfer_funds] Received a null `node` pointer. Exiting.");
        return TransferFundsResult::from_error(OperationStatus::NullPointer);
    }
    if arguments.is_null() {
        tracing::error!("[transfer_funds] Received a null `arguments` pointer. Exiting.");
        return TransferFundsResult::from_error(OperationStatus::NullPointer);
    }
    let arguments = unsafe { &*arguments };
    if let Err((error_message, status)) = unsafe { arguments.validate() } {
        tracing::error!("[transfer_funds] {error_message} Exiting.");
    ...
    let tip = if arguments.optional_tip.is_null() {
        match get_cryptarchia_info_sync(node) {
            Ok(cryptarchia_info) => cryptarchia_info.tip,
            Err(status) => {
                tracing::error!("[transfer_funds] Failed to get cryptarchia info. Aborting.");
                return TransferFundsResult::from_error(status);
            }
        }
    } else {
