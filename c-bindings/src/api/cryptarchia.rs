...
pub(crate) fn get_cryptarchia_info_sync(
    ...
    let Ok(runtime) = tokio::runtime::Runtime::new() else {
        tracing::error!("[get_cryptarchia_info_sync] Failed to create tokio runtime. Aborting.");
    ...
    let Ok(cryptarchia_info) = runtime.block_on(lb_api_service::http::consensus::cryptarchia_info(
        ...
        tracing::error!("[get_cryptarchia_info_sync] Failed to get cryptarchia info. Aborting.");
...
pub unsafe extern "C" fn get_cryptarchia_info(
    ...
    if node.is_null() {
        tracing::error!("[get_cryptarchia_info] Received a null `node` pointer. Exiting.");
