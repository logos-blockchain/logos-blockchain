        ...
        tokio::select! {
            Some(ServiceMessage::Blend(message_payload)) = inbound_relay.next() => {
                ...
                for _ in 0..message_copies {
                    recovery_checkpoint = handle_serialized_local_data_message(&serialized_data_message, &mut crypto_processor, &mut message_scheduler, recovery_checkpoint, public_info.session.session_number).await;
                }
            }
            ...
            Some(round_info) = message_scheduler.next() => {
                recovery_checkpoint = handle_release_round(round_info, &mut crypto_processor, rng, backend, network_adapter, public_info.session.session_number, recovery_checkpoint).await;
            }
    ...
    let processed_message = match remaining_message_type {
        ...
        DecapsulatedMessageType::Incompleted(remaining_encapsulated_message) => {
            ...
            ProcessedMessage::from(
                SessionBoundEncapsulatedMessageWithVerifiedHeader::from_components(
...
where
    ...
    ProofsVerifier: ProofsVerifierTrait,
{
    let (inner_message, session) = validated_encapsulated_message.into_components();
    ...
    match cryptographic_processor
        .validate_message_poq(inner_message.clone())
        ...
        Err(current_session_error) => {
            ...
            match old_crypto_processor
                .validate_message_poq(inner_message)
                .and_then(|message_with_verified_header| {
                    old_crypto_processor.decapsulate_message_recursive(message_with_verified_header)
                }) {
...
>(
    ...
    ProofsVerifier: ProofsVerifierTrait,
{
    let (inner_message, message_session) = validated_encapsulated_message.into_components();
    match cryptographic_processor
        .validate_message_poq(inner_message)
        .and_then(|message_with_verified_header| {
            cryptographic_processor.decapsulate_message_recursive(message_with_verified_header)
    ...
    ProofsVerifier: ProofsVerifierTrait,
{
    ...
    match decapsulated_message_type {
        ...
        DecapsulatedMessageType::Incompleted(remaining_encapsulated_message) => {
            ...
            let processed_message = ProcessedMessage::from(
                SessionBoundEncapsulatedMessageWithVerifiedHeader::from_components(
                    validated_message,
                    message_session,
                ),
    ...
    let data_messages_relay_futures = data_messages.into_iter()
        ...
        .inspect(|data_message_to_blend| {
            ...
            |data_message_to_blend| -> BoxFuture<'_, ()> {
                backend.publish(SessionBoundEncapsulatedMessageWithVerifiedHeader::from_components(data_message_to_blend, current_session)).boxed()
            },
        ).collect::<Vec<_>>();
