        ...
        + 'static,
{
    ...
    async fn request_blocks_from_peers(
        ...
        let eligible_connected_peers = connected_peers.intersection(&chainsync_peers).count();
        let eligible_discovered_peers = discovered_peers.intersection(&chainsync_peers).count();
        tracing::debug!(
            target: LOG_TARGET,
            "Selecting peers for target block {target_block:?} from local tip {local_tip:?} with immutable block {latest_immutable_block:?}; selected_peers={peers_to_request:?}, connected={}, discovered={}, eligible_connected={}, eligible_discovered={}, additional_blocks={}",
            connected_peers.len(),
            discovered_peers.len(),
            eligible_connected_peers,
            eligible_discovered_peers,
            additional_blocks.len()
        );

        if peers_to_request.is_empty() {
            return Err(format!(
                "no candidate peers available to download orphan ancestors for target block {target_block:?} (connected={}, discovered={}, eligible_connected={}, eligible_discovered={})",
                connected_peers.len(),
                discovered_peers.len(),
                eligible_connected_peers,
                eligible_discovered_peers
