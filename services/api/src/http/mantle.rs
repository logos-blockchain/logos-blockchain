...
{
    let new_blocks_receiver = CryptarchiaServiceApi::<Service>::from_relay(handle.relay().await?)
...
{
    let chain_api =
        CryptarchiaServiceApi::<Cryptarchia<RuntimeServiceId>>::from_relay(handle.relay().await?);
