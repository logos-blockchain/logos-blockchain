 ...
 * dereferencing the pointer.
 */
typedef struct PointerResult_NomosNode__OperationStatus {
  struct NomosNode *value;
  enum OperationStatus error;
} PointerResult_NomosNode__OperationStatus;

typedef struct PointerResult_NomosNode__OperationStatus InitializedNomosNodeResult;
 ...
 * function.
 */
CryptarchiaInfoResult get_cryptarchia_info(const struct NomosNode *node);
 ...
 */
InitializedNomosNodeResult start_nomos_node(const char *config_path);
 ...
 */
enum OperationStatus stop_node(struct NomosNode *node);

void subscribe_to_new_blocks(const struct NomosNode *node,
 ...
 */
BalanceResult get_balance(const struct NomosNode *node,
 ...
 * function.
 */
TransferFundsResult transfer_funds(const struct NomosNode *node,
