...
use crate::hd::{ExtendedSecretKey, HardenedIndex, MasterKey, MasterSeed, ZK_KEY_DST};

// Test vectors of the spec, derived from the BIP-39 mnemonic
...
pub(super) fn master() -> MasterKey {
    let seed = MasterSeed(hex::decode(SEED).unwrap().try_into().unwrap());
    seed.to_key()
}
