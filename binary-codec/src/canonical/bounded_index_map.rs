//! Canonical codec for [`BoundedIndexMap`]: a `MAX`-width length prefix, then
//! each entry as its key followed by its value, in the map's own order.
//!
//! The order is part of the value, so decoding accepts any order and keeps it.
//! A repeated key is still rejected (see [`super::keyed`]).
...
use super::{
    ...
    keyed::{cycled_fixture, distinct_fixtures_in_declared_order},
...
// Keys are self-describing (`Context = ()`); the context, if any, is the
// value's, as the element's is for a `BoundedVec`.
...
// them, which need not be sorted; values cycle through the value type's
// fixtures, so any value type works.
...
mod tests {
    ...
    type Map = BoundedIndexMap<u16, u8, 2, 4>;
    ...
    fn encode_keeps_insertion_order() {
        // Count `02`, then `0100` `aa`, then `0001` `bb`: not byte order.
        assert_eq!(
    ...
    #[test]
    fn different_orders_encode_differently() {
        assert_ne!(
            map(&[(1, 10), (2, 20)]).encode(),
            map(&[(2, 20), (1, 10)]).encode()
        );
    }
    ...
    /// Every key of a zero-length type encodes to the same empty slice, so the
    ...
    fn a_zero_length_key_type_cannot_drive_an_unbounded_loop() {
        type ZeroLength = BoundedIndexMap<[u8; 0], u8, 0, { u64::MAX as usize }>;
    ...
    fn a_large_declared_length_does_not_preallocate_from_the_wire() {
        type Wide = BoundedIndexMap<u64, u64, 1, { u16::MAX as usize }>;
    ...
    fn fixtures_keep_the_declared_order_of_the_key_fixtures() {
        // Keys `07` then `00`, as `u8` declares them, which is not byte order;
        // values cycle `07` then `00`.
        let fixtures = BoundedIndexMap::<u8, u8, 0, 4>::fixtures();
    ...
    fn fixtures_panic_when_the_key_has_too_few_distinct_fixtures() {
        drop(BoundedIndexMap::<u8, u8, 3, 4>::fixtures());
    }
}
