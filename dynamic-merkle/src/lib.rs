...
pub trait MerkleHasher {
    ...
    const EMPTY_VALUE: Self::Item;
    fn hash(item: &Self::Item) -> Self::Hash;
    fn compress(items: &[Self::Item]) -> Self::Hash;
}

const TREE_HEIGHT_EXCEPT_ROOT: usize = 32;

fn empty_subtree_root<Hasher: MerkleHasher>(height: usize) -> Hasher::Item {
    static PRECOMPUTED_EMPTY_ROOTS: OnceLock<
        [<Hasher as MerkleHasher>::Item; TREE_HEIGHT_EXCEPT_ROOT + 1],
    > = OnceLock::new();
    assert!(
    ...
    PRECOMPUTED_EMPTY_ROOTS.get_or_init(|| {
        let mut hashes = [Hasher::EMPTY_VALUE; TREE_HEIGHT_EXCEPT_ROOT + 1];
        for i in 1..=TREE_HEIGHT_EXCEPT_ROOT {
            hashes[i] = Hasher::compress(&[hashes[i - 1], hashes[i - 1]]);
        }
        hashes
    })[height]
}
...
fn hash<Hasher: MerkleHasher>(left: &Node<Hasher::Item>, right: &Node<Hasher::Item>) -> Hasher::Hash
where
    Hasher::Item: Copy,
{
    let mut input = [Hasher::EMPTY_VALUE; 2];
    match left {
        Node::Inner { value, .. } => input[0] = *value,
        Node::Leaf { item } => {
            input[0] = *item.as_ref().map_or(&Hasher::EMPTY_VALUE, AsRef::as_ref);
        }
    ...
    match right {
        ...
        Node::Leaf { item } => {
            input[1] = *item.as_ref().map_or(&Hasher::EMPTY_VALUE, AsRef::as_ref);
        }
        Node::Empty { height } => {
            input[1] = empty_subtree_root::<Hasher>(*height);
        }
    }
