...
pub struct MultiSequencerSignature {
...
impl From<(ChannelKeyIndex, Ed25519Signature)> for MultiSequencerSignature {
    fn from((index, signature): (ChannelKeyIndex, Ed25519Signature)) -> Self {
        Self::new(index, signature)
    }
}

impl PartialOrd<Self> for MultiSequencerSignature {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for MultiSequencerSignature {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct ChannelMultiSequencerProof {
    // Invariant: signatures are sorted by index (then signature) with no duplicates
    signatures: Vec<MultiSequencerSignature>,
}

impl ChannelMultiSequencerProof {
    pub fn new(signatures: Vec<MultiSequencerSignature>) -> Result<Self, Error> {
    ...
    fn normalize_signatures(
        mut signatures: Vec<MultiSequencerSignature>,
    ) -> Vec<MultiSequencerSignature> {
    ...
    fn validate_well_formedness(signatures: &[MultiSequencerSignature]) -> Result<(), Error> {
    ...
    #[must_use]
    pub const fn signatures(&self) -> &Vec<MultiSequencerSignature> {
        &self.signatures
    }
}

impl TryFrom<Vec<MultiSequencerSignature>> for ChannelMultiSequencerProof {
    type Error = Error;

    fn try_from(value: Vec<MultiSequencerSignature>) -> Result<Self, Self::Error> {
        Self::new(value)
    }
}
