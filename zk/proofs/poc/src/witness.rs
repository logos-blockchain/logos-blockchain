...
static BINARY: LazyLock<PathBuf> =
    LazyLock::new(|| logos_blockchain_circuits_utils::witness_generator_path(CIRCUIT_NAME));
...
pub fn generate_witness(inputs: &PoCWitnessInputs) -> Result<Witness, std::io::Error> {
    ...
    logos_blockchain_witness_generator::generate_witness(&str_inputs, BINARY.as_path()).map(Witness)
}
