...
pub fn prove(inputs: PoQWitnessInputs) -> Result<(PoQProof, PoQVerifierInput), ProveError> {
    ...
    let (proof, verifier_inputs) =
        logos_blockchain_circuits_prover::prover_from_contents(POQ_PROVING_KEY_PATH.as_path(), witness.as_ref())
...
pub fn verify(proof: &PoQProof, public_inputs: PoQVerifierInput) -> Result<bool, VerifyError> {
    ...
    logos_blockchain_groth16::groth16_verify(verification_key::POQ_VK.as_ref(), &expanded_proof, &inputs)
