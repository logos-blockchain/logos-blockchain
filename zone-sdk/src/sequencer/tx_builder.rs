...
mod tests {
    ...
    fn transfer_op() -> Op {
        let pk = ZkKey::from(BigUint::from(0u64)).to_public_key();
        let utxo = Utxo {
            op_id: [0; 32],
            output_index: 0,
            note: Note::new(10, pk),
        ...
        Op::Transfer(TransferOp {
            ...
            outputs: Outputs::new([Note::new(1, pk)]),
        })
    }

    fn inscribe_op(signing_key: &Ed25519Key) -> Op {
        Op::ChannelInscribe(InscriptionOp {
            channel_id: ChannelId::from([0; 32]),
            inscription: b"payload".into(),
            parent: MsgId::root(),
            signer: signing_key.public_key(),
        })
    }

    fn transfer_proof(tx: &MantleTx) -> OpProof {
        let sk = ZkKey::from(BigUint::from(0u64));
        OpProof::ZkSig(ZkKey::multi_sign(std::slice::from_ref(&sk), &tx.hash().to_fr()).unwrap())
    }

    #[test]
    fn attach_transfer_proof_appends_for_funded_tx() {
        let signing_key = Ed25519Key::from_bytes(&[0; 32]);
        let tx = MantleTx([inscribe_op(&signing_key), transfer_op()].into());
        let own_proof = OpProof::Ed25519Sig(sign_tx(tx.hash(), &signing_key));

        let proofs =
            attach_transfer_proof(&tx, vec![own_proof], Some(transfer_proof(&tx))).unwrap();

        assert_eq!(proofs.len(), 2);
        assert!(matches!(&proofs[0], OpProof::Ed25519Sig(_)));
        assert!(matches!(&proofs[1], OpProof::ZkSig(_)));
    }

    #[test]
    fn attach_transfer_proof_keeps_feeless_tx_unchanged() {
        let signing_key = Ed25519Key::from_bytes(&[0; 32]);
        let tx = MantleTx([inscribe_op(&signing_key)].into());
        let own_proof = OpProof::Ed25519Sig(sign_tx(tx.hash(), &signing_key));

        let proofs = attach_transfer_proof(&tx, vec![own_proof], None).unwrap();

        assert_eq!(proofs.len(), 1);
    }

    #[test]
    fn attach_transfer_proof_rejects_funded_tx_without_proof() {
        let signing_key = Ed25519Key::from_bytes(&[0; 32]);
        let tx = MantleTx([inscribe_op(&signing_key), transfer_op()].into());
        let own_proof = OpProof::Ed25519Sig(sign_tx(tx.hash(), &signing_key));

        assert!(attach_transfer_proof(&tx, vec![own_proof], None).is_err());
    }

    #[test]
    fn atomic_withdraw_proofs_cover_fee_transfer() {
        let signing_key = Ed25519Key::from_bytes(&[0; 32]);
        let withdraw = Op::ChannelWithdraw(ChannelWithdrawOp {
            channel_id: ChannelId::from([0; 32]),
            outputs: Outputs::new([Note::new(
                1,
                ZkKey::from(BigUint::from(0u64)).to_public_key(),
            )]),
            withdraw_nonce: 0,
        });
        let tx = MantleTx([withdraw, inscribe_op(&signing_key), transfer_op()].into());
        let own_sig = sign_tx(tx.hash(), &signing_key);

        let proofs =
            build_atomic_withdraw_ops_proofs(&tx, 0, own_sig, Some(&transfer_proof(&tx))).unwrap();
        assert_eq!(proofs.len(), 3);
        assert!(matches!(&proofs[0], OpProof::ChannelMultiSigProof(_)));
        assert!(matches!(&proofs[1], OpProof::Ed25519Sig(_)));
        assert!(matches!(&proofs[2], OpProof::ZkSig(_)));

        assert!(build_atomic_withdraw_ops_proofs(&tx, 0, own_sig, None).is_err());
    }
}
